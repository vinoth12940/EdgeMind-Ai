...
for m in matches:
    ...
    if name_m and var_m:
        ...
        catalog[eff_id] = (name, var, rt_m.group(1) if rt_m else "gguf")

gguf_profiles = []
for p in profiles:
    cid = p["catalogID"].upper()
    if cid in catalog:
        name, var, rt = catalog[cid]
        if rt == "gguf":
            gguf_profiles.append((name, var, p))

print(f"Total GGUF profiles: {len(gguf_profiles)}")
for name, var, p in gguf_profiles[:5]:
    print(f"\n{name} [{var}]:")
