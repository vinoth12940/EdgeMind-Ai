...
- **Current App Store state (September 15, 2026):** version `0.3.2`, build `8` is the current release train. It carries the whole 0.3.1 feature set (regenerate/edit, export, model suggestions, memory, document library, Share Extension, widgets, Shortcuts answers) plus Apple Intelligence vision and tool calling on iOS 27, on-device OCR for scanned PDFs, relevance-based document excerpting, and prompt budgeting that stops LiteRT overflowing its 2048-token cap. Version `0.3.1` build `7` was **approved and is live** (`READY_FOR_SALE`), so its train is closed to new builds. Version `0.3.0` build `6` was approved earlier, and `0.2.0` build `5` was submitted on July 8, 2026. Both numbers live in `project.yml`; bump them there, then run `xcodegen generate` before archiving. `MARKETING_VERSION` is user-facing (`CFBundleShortVersionString`), `CURRENT_PROJECT_VERSION` is the build number (`CFBundleVersion`) and must increment on every new upload.
