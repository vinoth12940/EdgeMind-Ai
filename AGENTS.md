...
- **Current App Store state (September 19, 2026):** version `0.3.4`, build `11` is the **launch-crash hotfix** — it bounds inline Markdown rendering so a long restored chat message can no longer overflow the 1 MB main-thread stack (see the "One `Text` per character" gotcha). Build `11` is built and verified on device but **not yet uploaded**. Version `0.3.3`, build `10` was **approved and is live** (`READY_FOR_SALE`, auto-released `AFTER_APPROVAL`; version ID `1927fb0b-3eb7-407e-b4d5-d4afec7647b7`, review submission `030c543d-1d73-45f5-ba99-df63ea36928f`, Delivery UUID `db3c68d0-900a-48a4-87b8-f913d99a152d`), so its train is now closed to new builds. It carries the whole 0.3.1 feature set (regenerate/edit, export, model suggestions, memory, document library, Share Extension, widgets, Shortcuts answers), Apple Intelligence vision and tool calling on iOS 27, on-device OCR, relevance-based document excerpting and LiteRT prompt budgeting, **plus 28 fixes from a whole-codebase audit** (tool calling no longer fires on greetings; Apple Intelligence and Gemma/LFM reach the document library; a frozen-UI Markdown loop; PDF export pagination; answer-version and regenerate data loss; Keychain credential loss on launch; share double-import; MLX install-state and cache path; LiteRT search grounding and prompt budget; light-mode contrast). Version `0.3.2` build `9` was **approved and is live** (`READY_FOR_SALE`), so its train is closed. Version `0.3.1` build `7` and `0.3.0` build `6` were approved earlier, and `0.2.0` build `5` was submitted on July 8, 2026. Both numbers live in `project.yml`; bump them there, then run `xcodegen generate` **and commit the regenerated `EdgeMindAi.xcodeproj/project.pbxproj`** before archiving — the build number only reaches the build through regeneration. `MARKETING_VERSION` is user-facing (`CFBundleShortVersionString`), `CURRENT_PROJECT_VERSION` is the build number (`CFBundleVersion`) and must increment on every new upload. It carries the whole 0.3.1 feature set (regenerate/edit, export, model suggestions, memory, document library, Share Extension, widgets, Shortcuts answers), Apple Intelligence vision and tool calling on iOS 27, on-device OCR, relevance-based document excerpting and LiteRT prompt budgeting, **plus 28 fixes from a whole-codebase audit** (tool calling no longer fires on greetings; Apple Intelligence and Gemma/LFM reach the document library; a frozen-UI Markdown loop; PDF export pagination; answer-version and regenerate data loss; Keychain credential loss on launch; share double-import; MLX install-state and cache path; LiteRT search grounding and prompt budget; light-mode contrast). Version `0.3.2` build `9` was **approved and is live** (`READY_FOR_SALE`), so its train is closed. Version `0.3.1` build `7` and `0.3.0` build `6` were approved earlier, and `0.2.0` build `5` was submitted on July 8, 2026. Both numbers live in `project.yml`; bump them there, then run `xcodegen generate` **and commit the regenerated `EdgeMindAi.xcodeproj/project.pbxproj`** before archiving — the build number only reaches the build through regeneration. `MARKETING_VERSION` is user-facing (`CFBundleShortVersionString`), `CURRENT_PROJECT_VERSION` is the build number (`CFBundleVersion`) and must increment on every new upload.
...
- **One `Text` per character overflows the main-thread stack (0.3.4 launch-crash hotfix)**: `MarkdownTextView.inlineMarkdown` used to append one `Text` per plain character. SwiftUI stores `a + b` as a left-nested `ConcatenatedTextStorage`, and *resolving* it recurses **once per node**, so a single rendered line longer than roughly **670 characters** exhausted the 1 MB main-thread stack on device and killed the app with `SIGSEGV` — "Thread stack size exceeded due to excessive recursion" — while restoring a saved chat. That was the reported "app crashes when I open it": a 630-character assistant reply in the restored session, just past the cliff once the real view hierarchy is in play (15 identical device crash reports). `InlineMarkdownParser` now coalesces consecutive plain characters into one run and hard-caps styled runs at `InlineMarkdownParser.maxStyledRuns` (256); past the cap the remainder renders unstyled but is never dropped. **Never reintroduce per-character text accumulation**, and when adding inline styling assert on **run count**, not string length. The trap is silent in CI: the simulator's main thread has **8 MB**, so it survives ~5,450 runs and only the device's 1 MB stack overflows — a green simulator run proves nothing here (same shape as the MLX/LiteRT `#else`-branch gotcha below). `MarkdownTextViewStackSafetyTests` renders 37 kB messages through a real `UIHostingController` and does fail against the old behaviour.
