...
- **Current App Store state (September 14, 2026):** version `0.3.1`, build `7` was uploaded, attached, and submitted for review on September 14, 2026 via the App Store Connect REST API (review submission ID `09fccb09-4a7f-4985-a36b-de5c5c546fd4`, version ID `0bc48973-412f-4196-b76b-c9a56c9fff6f`, state `WAITING_FOR_REVIEW`). Version `0.3.0` build `6` was **approved** (the 0.3.0 train is closed to new builds). Build 7 fixes a launch hang on iOS 26.6.1 devices (`.defaultScrollAnchor(.bottom)` on the chat `LazyVStack` looped at first render and tripped the scene-create watchdog whenever a saved chat existed), plus chat state/Stop/tool-loop bugs. Build `6` (0.3.0) was submitted on September 7, 2026 via the App Store Connect REST API (review submission ID `8ea92a89-e91e-4464-999c-e611a2335998`) and ships the launch hang. Version `0.2.0` build `5` was submitted on July 8, 2026. Both numbers live in `project.yml`; bump them there, then run `xcodegen generate` before archiving. `MARKETING_VERSION` is user-facing (`CFBundleShortVersionString`), `CURRENT_PROJECT_VERSION` is the build number (`CFBundleVersion`) and must increment on every new upload.
...
- **Embeddings** — `DocumentEmbedder` prefers `NLContextualEmbedding` (mean-pooled, only when `hasAvailableAssets`), then `NLEmbedding.sentenceEmbedding`, else `.none` (keyword-only). Queries are embedded with the same kind as the target document; if that kind is unavailable the document degrades to BM25 only.
- **Search** — `DocumentSearchService.search(query:index:limit:)` scores `0.7 × cosine + 0.3 × normalized BM25` (BM25 alone for `.none`), top 5 by default. `renderHits` labels passages `[fileName p.12]` and bounds total text by `InferenceBudget.documentContextBudget(for:)`.
- **Models** — `SearchDocumentsTool` (`search_documents`) is registered only when `AppSettings.documentSearchEnabled` (default on) **and** the index is non-empty; each hit becomes a citation-style source. Non-tool models get the same search upfront through `UpfrontToolDetector` when the prompt references the user's documents. Indexing is 100% on-device and no user content is sent anywhere.
...
- **Generation stats (0.3.0)**: `.done` carries a `GenerationStats` payload (time-to-first-token, total duration, token count). `StreamProcessor` accumulates wall-clock timing and a delta count via `StatsAccumulator` for every backend. Exact token counts are injected by an `exactTokenCountProvider` closure supplied at init — GGUF via `LocalLlamaRuntime.lastGeneratedTokenCount()` (`generatedTokenCount`), MLX via the previously-discarded `.info(GenerateCompletionInfo)` case. Backends that bypass `StreamProcessor` (`AppleFoundationModelService`) construct `GenerationStats` directly. Aborted streams (watchdog timeout, repetition guard) skip the exact-count provider and rely on the delta-count fallback.
...
### Key wiring points
- `EdgeMindAiApp.swift` (@main): creates the shared `AppStateStore` and `ChatTurnEngine`, injects `AppStateStore`, `AuthStateStore`, and `ChatTurnEngine` into the environment, gates `RootView` behind auth via `LaunchRootView`, and applies `.preferredColorScheme(store.settings.appearanceMode.preferredColorScheme)` at the window level. The engine is built from `ChatTurnEngine.Dependencies.live()`.
