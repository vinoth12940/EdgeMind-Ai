...
- **Current App Store state (September 16, 2026):** version `0.3.2`, build `9` is the current release train, submitted and **`WAITING_FOR_REVIEW`** (review submission `877406e8-5192-4bc9-b71c-982afa8886f1`, Delivery UUID `fe5bad17-ae55-4606-838a-4cda2dc261d6`). Build `9` carries the whole 0.3.1 feature set (regenerate/edit, export, model suggestions, memory, document library, Share Extension, widgets, Shortcuts answers) plus Apple Intelligence vision and tool calling on iOS 27, on-device OCR for scanned PDFs, relevance-based document excerpting, prompt budgeting that stops LiteRT overflowing its 2048-token cap, and the tool-calling/document-search fixes below. **Build `8` was withdrawn from review (`DEVELOPER_REJECTED`) because it shipped three regressions** — tools firing on greetings, `search_documents` rejecting Apple Intelligence's bare-string arguments, and flat tool-call payloads losing their arguments. Version `0.3.1` build `7` was approved and is live (`READY_FOR_SALE`), so its train is closed to new builds. Version `0.3.0` build `6` was approved earlier, and `0.2.0` build `5` was submitted on July 8, 2026. Both numbers live in `project.yml`; bump them there, then run `xcodegen generate` **and commit the regenerated `EdgeMindAi.xcodeproj/project.pbxproj`** before archiving — the build number only reaches the build through regeneration. `MARKETING_VERSION` is user-facing (`CFBundleShortVersionString`), `CURRENT_PROJECT_VERSION` is the build number (`CFBundleVersion`) and must increment on every new upload.
