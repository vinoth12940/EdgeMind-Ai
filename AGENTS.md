...
- **Current App Store state (September 7, 2026):** version `0.3.0`, build `6` has been uploaded, build attached, and submitted for review via the App Store Connect REST API (review submission ID `8ea92a89-e91e-4464-999c-e611a2335998`, state `WAITING_FOR_REVIEW`). Version `0.2.0` build `5` was submitted on July 8, 2026. Version `0.1.0` remains the approved/live release until Apple approves a newer version. Both numbers live in `project.yml`; bump them there, then run `xcodegen generate` before archiving. `MARKETING_VERSION` is user-facing (`CFBundleShortVersionString`), `CURRENT_PROJECT_VERSION` is the build number (`CFBundleVersion`) and must increment on every new upload.
